	...
	while (current != NULL) {
		...
		/* IP Header */
    		...
    		new_ip_hdr->ip_sum = cksum(buf + ethernet_hdr_size, ip_hdr_size); 
    	        ...
    	        /* UDP Header */
		sr_ip_hdr_t *udp_hdr = (sr_udp_hdr_t*)(buf + ethernet_hdr_size + ip_hdr_size);
		udp_hdr->port_dst = 520;
		udp_hdr->port_dst = 520;
		udp_hdr-> udp_len = htons(len - ethernet_hdr_size - ip_hdr_size);

		/* RIP Packet */
		sr_ip_hdr_t *rip_pkt = (sr_rip_pkt_t*)(buf + ethernet_hdr_size + ip_hdr_size + udp_hdr_size);
		rip_pkt->command = 2;
		rip_pkt->version = 2;
		struct entry *my_rip_entry = (struct entry*)(rip_packet + 2*sizeof(uint8_t) + sizeof(uint16_t) )
